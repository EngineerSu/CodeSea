...
A: 遍历矩阵中的点,当遇到和字符串第一个字符匹配的时候,开启匹配方法:从第一个位置上下左右走,如果可以走继续走,如果不可以走,准备回退. 这种回溯需要一个和矩阵大小一样的boolean数组,标志着已经走过的路,不能回走.并且在行走时,需要用stack将路线压栈,回退时,要将栈顶的坐标做一个标记,让它知道回退后不能再往相同方向走.
