...
A2：还有一个简单解，下一层每一个数都需要+上一层的最小值，这样层层计算下去，最后一层的最小值即是答案。思路相当于将路径的最小值和一直往下层携带
...
[子数组的最大乘积](https://leetcode-cn.com/problems/maximum-product-subarray/)

Q：给一个数组arr(可能包含正数负数0),求arr的最大子数组乘积

A：使用curMax和curMin记录以 i 结尾的子数组的最大乘积和最小乘积，那么在每次遍历更新这两个值时，为以下递推式：
...
结果是所有curMax的最大值。这种保持最大最小两个状态值，在乘法的时候很常见，要注意更新的时候区别看待
...
A：也使用`dp[i][j]`表示以 i 和 j 结尾的最长公共子串，由于子串不同于子序列，所以它们的递推式有变化：
